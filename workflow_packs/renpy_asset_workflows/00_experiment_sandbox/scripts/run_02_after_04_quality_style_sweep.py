...
def first(h,nid):
    for node_id,node in h.get('outputs',{}).items():
        for img in node.get('images',[]):
            if node_id==nid: return str(COMFY/'output'/(img.get('subfolder') or '')/img['filename'])
    raise RuntimeError(h.get('outputs',{}))

srcs={
 'q01_crisp_ip065':'/mnt/c/Users/Desktop/Documents/ComfyUI/output/hermes_vn_04_quality_style_sweep_20260513/04_q01_crisp_ip065_seed719252501_00001_.png',
 'q01_crisp_ip075':'/mnt/c/Users/Desktop/Documents/ComfyUI/output/hermes_vn_04_quality_style_sweep_20260513/04_q01_crisp_ip075_seed719252501_00001_.png',
}
manifest={'run':RUN,'alpha_variants':[]}
for slug,src in srcs.items():
    dst=INDIR/f'04_{slug}.png'; shutil.copy2(src,dst)
    ...
    p['3']['inputs']['filename_prefix']=f'{RUN}/02_alpha_after_04_{slug}_b1_ref1'
    pid,h=submit(p)
    manifest['alpha_variants'].append({'slug':slug,'prompt_id':pid,'source':src,'alpha_output':first(h,'3')})
mp=COMFY/'output'/RUN/'manifest_02_after_quality_style_sweep.json'
