...
RUN='hermes_vn_04_alpha_minus_head_upperclip_20260513'
...
def make_clipped_alpha(name, ycut):
    dst=INDIR/name
    # Extract alpha, harden, then black out lower area to preserve skirt/legs more.
    vf=f'alphaextract,format=gray,lut=y=if(gte(val\\,16)\\,255\\,0),format=rgb24,drawbox=x=0:y={ycut}:w=iw:h=ih-{ycut}:color=black:t=fill'
...
def make_contact(manifest):
    ...
    for v in manifest['variants']:
        files += [Path([o['path'] for o in v['outputs'] if o['node']=='30'][0]), Path([o['path'] for o in v['outputs'] if o['node']=='33'][0])]
    inputs=[]; labels=[]
    ...
    dst=OUTDIR/'contact_upperclip.png'
...
def main():
    ...
    variants=[
        {'ycut':1180,'grow':28,'denoise':0.88,'seed':719254291,'slug':'y1180_g28_d88'},
        {'ycut':1240,'grow':28,'denoise':0.88,'seed':719254291,'slug':'y1240_g28_d88'},
        {'ycut':1240,'grow':32,'denoise':0.88,'seed':719254291,'slug':'y1240_g32_d88'},
        {'ycut':1320,'grow':28,'denoise':0.88,'seed':719254291,'slug':'y1320_g28_d88'},
    ]
    manifest={'run':RUN,'purpose':'Try clipping the edit alpha mask below skirt/leg area to preserve lower outfit while still allowing upper body/side hand cleanup.','variants':[]}
    for v in variants:
        maskrel=make_clipped_alpha(f'alpha_clip_{v["slug"]}.png',v['ycut'])
        p=json.loads(json.dumps(base))
        p['1']['inputs']['image']=f'{RUN}/source01.png'; p['2']['inputs']['image']=f'{RUN}/pose_arms_crossed.png'; p['3']['inputs']['image']=maskrel
        if '34' in p: p['34']['inputs']['expand']=v['grow']
        p['24']['inputs']['denoise']=v['denoise']; p['24']['inputs']['seed']=v['seed']
        p['13']['inputs']['expand']=6; p['14']['inputs']['kernel_size']=8; p['14']['inputs']['sigma']=4.0
        for nid,prefix in [('29','protect_headhairface'),('30','edit_alpha_minus_head'),('31','openpose'),('32','raw'),('33','composite')]:
            p[nid]['inputs']['filename_prefix']=f'{RUN}/{prefix}_{v["slug"]}'
        wf=ROOT/'00_experiment_sandbox/workflow_api'/f'00_04_alpha_minus_head_upperclip_{v["slug"]}_api.json'
        wf.write_text(json.dumps(p,indent=2,ensure_ascii=False),encoding='utf-8')
        pid,h=submit(p)
        item=dict(v); item.update({'alpha_mask_input':maskrel,'prompt_id':pid,'workflow_api':str(wf),'outputs':outputs(h)})
        manifest['variants'].append(item)
    manifest['contact_sheet']=make_contact(manifest)
    mp=OUTDIR/'manifest_upperclip.json'; mp.write_text(json.dumps(manifest,indent=2,ensure_ascii=False),encoding='utf-8')
